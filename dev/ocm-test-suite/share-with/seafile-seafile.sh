...
main() {
    ...
    create_seafile   1      "jonathan@seafile.com"    "xu"             seafileltd/seafile-mc    "${EFSS_PLATFORM_1_VERSION}"    "seafile2"
    create_seafile   2      "giuseppe@cern.ch"        "lopresti"       seafileltd/seafile-mc    "${EFSS_PLATFORM_2_VERSION}"    "seafile1"

    if [ "${SCRIPT_MODE}" = "dev" ]; then
        run_dev \
            "https://seafile1.docker (username: jonathan@seafile.com, password: xu)" \
            "https://seafile2.docker (username: giuseppe@cern.ch, password: lopresti)"
    else
