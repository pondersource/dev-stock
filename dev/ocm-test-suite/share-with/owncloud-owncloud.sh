...
# -----------------------------------------------------------------------------------
# Script to Test ownCloud to ownCloud OCM share-with flow tests.
...
main() {
    ...
    if [ "${SCRIPT_MODE}" = "dev" ]; then
        ...
        docker run --network="${DOCKER_NETWORK}" \
            ...
            --spec "cypress/e2e/share-with/owncloud-${P1_VER}-to-owncloud-${P2_VER}.cy.js" || error_exit "Cypress tests failed."
