...
docker run -d --network=testnet --name=revad1.docker -v /workspace/reva:/reva -e HOST=revad1 pondersource/dev-stock-revad
...
docker run -d --network=testnet --name=revad2.docker -v /workspace/reva:/reva -e HOST=revad2 pondersource/dev-stock-revad
