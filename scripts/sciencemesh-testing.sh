...
docker run --restart=always -d --network=testnet --name=revaoc1.docker -e HOST=revaoc1 pondersource/dev-stock-revad
...
docker run --restart=always -d --network=testnet --name=oc1.docker pondersource/dev-stock-oc1-sciencemesh
docker run --restart=always -d --network=testnet --name=revaoc2.docker -e HOST=revaoc2 pondersource/dev-stock-revad
...
docker run --restart=always -d --network=testnet --name=oc2.docker pondersource/dev-stock-oc2-sciencemesh
...
echo Click it to go to the meshdir server, and choose oc2 there.
echo Log in on https://oc2.docker as marie / radioactivity
